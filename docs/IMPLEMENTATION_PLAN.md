...
当前进度：已完成。微信真实 Client 与 dev/test 显式 mock、双端固定 TTL 会话、商家失败锁定、Token 域隔离、退出登录、资料查询和改密后全部会话失效均已落地；prod 配置强制关闭 mock。
...
当前进度：已完成。商家店铺设置与营业/堂食/配送独立开关、公开店铺 Redis 缓存、分类增改启停删除、用户商品分页与详情、商家商品 CRUD/独立状态动作、规格口味同步和本地安全图片存储均已落地；所有商家数据条件取自会话 `shopId`，prod 可通过挂载目录持久化上传文件。
...
当前进度：已完成。购物车当前价格预览、同商品规格口味组合合并、本人条目增删改查与清空、商品及选项有效性校验、地址 CRUD/逻辑删除和唯一默认地址均已落地；购物车合并与默认地址写操作通过当前用户数据库行锁串行化，并覆盖数量边界、数据归属和默认地址并发测试。
