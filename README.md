...
单店铺微信点餐小程序的 Spring Boot 后端。当前已完成 M1—M8，包括基础工程、数据库、双端登录与权限、店铺/商品、购物车/地址、安全下单、订单查询与完整状态流转，以及联调质量和生产部署材料。
...
生产环境默认关闭 Swagger。示例账号、联系电话、地址和图片路径必须在部署前替换。生产镜像、HTTPS、持久化、备份恢复和微信域名配置见 [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md)，生产启动使用 `compose.prod.yml`；`compose.yml` 仅用于本地 MySQL/Redis。
