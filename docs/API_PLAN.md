...
| POST `/api/common/files/images` | MERCHANT | 图片上传；实施时也可固定到商家路径，但只保留一种 |
...
| 40000 | 商品分类 | 40001 不存在、40002 下架、40003 售罄、40004 库存不足 |
| 50000 | 购物车地址 | 50001 购物车为空、50002 地址不存在 |
...
- 下单：客户端随机键 + Redis 原子占位 + `orders(user_id,idempotency_key)` 唯一约束；成功重试返回同一订单，处理中返回 409。
